...
class RemoteService
{
  ...
  Future<dynamic> uploadFile(String endPoint, String fileName, String filePath, File? file, Uint8List? fileBytes, BuildContext ctx) async{
    ...
    try{
      ...
      try {
        formdata = Dioo.FormData.fromMap({
          "file": (!kIsWeb ? await Dioo.MultipartFile.fromFile(
            file!.path,
            filename: fileName,
        ...
        ScaffoldMessenger.of(ctx).showSnackBar(
            SnackBar(content: Text(
                "Oups, la validation du fichier a échoué, vous pouvez recommencer.")));

        return Future<dynamic>.value(null);
      }
      Dioo.Dio dio = Dioo.Dio();
      dio.options.headers["Authorization"] = "Bearer ${jwt ?? ""}";
      print("${Convers.api_url}/api/v2/generic/picture/$endPoint");
      print(Dioo.FormData);
      var response = await dio.post("${Convers.api_url}/api/v2/generic/picture/$endPoint",
        data: formdata ?? "",
      ...
      if (response.statusCode == 200)
      {
        ...
        if (jsonData.containsKey("errorMessage")) {
          ...
          ScaffoldMessenger.of(ctx).showSnackBar(
              SnackBar(content: Text("Oups, la validation n'a pas marché, vous pouvez recommencer.")));
        }
        return Future<dynamic>.value(null);
      }
    }catch(error) {
      ...
      ScaffoldMessenger.of(ctx).showSnackBar(
          SnackBar(content: Text(
              "Oups, la validation n'a pas marché, vous pouvez recommencer.")));
    }
    return Future<dynamic>.value(null);
  }

  Future<dynamic> request(String type, String endpoint, bool authed, Map<String, String> customHeaders, Map<String, dynamic> postData,  BuildContext ctx) async{
    const storage = FlutterSecureStorage();
    String? jwt = await storage.read(key: "jwt");
    String version = "";
    String buildNumber = "";
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          try {
            if (jsonData.containsKey("id_token")) {
              await storage.write(
                  key: "jwt", value: jsonData["id_token"]);
            }
          ...
          if (jsonData.containsKey("detail")) {
            ScaffoldMessenger.of(ctx).showSnackBar(
                SnackBar(content: Text(jsonData["detail"])));
          } else if (jsonData.containsKey("errorMessage")) {
            ScaffoldMessenger.of(ctx).showSnackBar(
                SnackBar(content: Text(jsonData["errorMessage"])));
          } else if (jsonData.containsKey("title")) {
            ScaffoldMessenger.of(ctx).showSnackBar(
                SnackBar(content: Text(jsonData["title"])));
          } else if (jsonData.containsKey("result") && isArrayContainingMessage(jsonData["result"])) {
            ScaffoldMessenger.of(ctx).showSnackBar(
